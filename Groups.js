...
function cleanText(str) {
    if (!str) return "";
    let cleaned = str.replace(/[çÇ]/g, 'c').replace(/[ğĞ]/g, 'g').replace(/[ıİ]/g, 'i').replace(/[öÖ]/g, 'o').replace(/[şŞ]/g, 's').replace(/[üÜ]/g, 'u').trim();
    let prefix = "";
...
function createNameLi(name, gender, isManual = false) {
    ...
    if (isManual) {
        let l = raw.replace(/^\*+/, "").trim().toLowerCase().split(" ");
        let stripped = l.filter(w => w !== "").join(' ');
        
        let exists = false;
        $('.name-text').each(function(){ if($(this).text().replace(/^\*+/, "").trim().toLowerCase() === stripped) exists = true; });
        
        let cleaned = l.filter(w => w !== "").map(w => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()).join(" ");
    ...
    return $(`<li class="ui-state-default" data-gender="${gender}">
      ...
      <button class="btn-icon remove-name-btn">×</button></li>`);
}
...
function createGroupHeader(groupName, gender, count = 0) {
    ...
    return $(`<div class="group-title ${bgClass}">
        ...
        <div class="group-controls"><button class="btn-icon edit-group-btn">✎</button><button class="btn-icon delete-group-btn">🗑</button></div>
    </div>`);
}
...
$('#drawer-toggle').on('click', function() {
    ...
    $(this).text($('#floating-controls').hasClass('expanded') ? '✖' : '⚙');
});
...
$('#fileInput').on('change', function(e) {
    ...
    reader.onload = (evt) => {
        ...
        $('#floating-controls').removeClass('expanded'); $('#drawer-toggle').text('⚙');
    };
